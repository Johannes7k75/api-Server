let tableItems = document.getElementById("table").getElementsByClassName("item");
...
window.addEventListener("load", async function () {
    ...
    let data = await fetch("http://192.168.178.42:3000/items").then(res => res.json());
    data = data.sort((a, b) => a.name.localeCompare(b.name));
    for (let rowData of data) {
        if (rowData.rows.length === 1) {
            createNormalRow(rowData.name, rowData.rows[0].stockSize, rowData.rows[0].stock, rowData.id);
    ...
    for (let i = 0; i < this.document.getElementById("table").children.length; i++) {
            ...
            for (let j = 0; j < children.children.length; j++) {
                if (children.children[j].classList.contains("rows")) {
                    for (let k = 0; k < children.children[j].children.length; k++) {
                        let modulus = i % 2 === 1 ? k % 2 === 0 ? 1 : 0 : k % 2 === 0 ? 0 : 1;
                        children.children[j].children[k].classList.add(`rowType${modulus + 1}`);
                        if (i % 2 === 1 && k % 2 === 1 && children.children[j].children.length - 1 === k) {
                            children.children[j].children[k].setAttribute("style", "border-bottom-left-radius: 0px; ");
                        }
                    }
                }
            }
    ...
    this.document.getElementById("header-action-remove").addEventListener("click", clickMenuRemove, false);
    this.document.getElementById("header-action-edit").addEventListener("click", clickMenuEdit, false);
    // }
...
function clickMenuRemove(e) {
    ...
    document.getElementById("header-action-remove-confirm").addEventListener("click", clickMenuRemoveConfirm, false);
    for (let checkbox of ckeckboxes) {
        if (checkbox.style.display === "none") {
            checkbox.setAttribute("style", "");
            document.getElementById("header-action-remove-confirm").setAttribute("style", "display: block;");
        } else {
...
async function clickMenuRemoveConfirm(e) {
    let removeBoxesChecked = JSON.parse(localStorage.getItem("removeBoxesChecked"));
    ...
    localStorage.setItem("removeBoxesChecked", JSON.stringify(removeBoxesChecked));
    // document.getElementById("header-action-remove-confirm").setAttribute("style", "display: none;");
    // console.log(removeBoxesChecked);
}

function clickCheckbox(e) {
    const checkboxesChecked = JSON.parse(localStorage.getItem("removeBoxesChecked"));
    ...
    //     checkboxesChecked.push(e.target.id);
    // }
    console.log(e.target, checkboxesChecked.includes(e.target.id), checkboxesChecked);
...
function clickMenuAddRowAdd(e) {
    let menuDiv = e.srcElement.parentElement.parentElement.parentElement.parentElement;
    console.log(menuDiv);
